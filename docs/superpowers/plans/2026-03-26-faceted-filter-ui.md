# Faceted Filter Tarot Card UI Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the static Gradio form with a custom faceted-filter tarot card UI where users adjust filters to see matching cities as animated cards, pick 3 cards, and flip them to reveal cities before triggering the LLM detail guide.

**Architecture:** `recommender.py` is extended with `top_n` parameter and `compute_disabled_options()`. A new `ui/layout_v2.py` file exposes two Gradio API endpoints (`filter_cities`, `nomad_advisor`) and renders the full custom HTML/CSS/JS UI. `app.py` is toggled via `USE_NEW_UI=1` env var.

**Tech Stack:** Python 3.11, Gradio 4.x (`gr.Blocks`), vanilla JS (no framework), CSS `backdrop-filter`/keyframes.
...
| File | Role |
|------|------|
| `recommender.py` | Extend: `top_n` param, `compute_disabled_options()`, `_TIMELINE_FILTER` 6개월 entry |
| `ui/layout_v2.py` | New: `filter_cities()`, `nomad_advisor_v2()`, `build_layout_v2()`, all HTML/CSS/JS inlined |
| `app.py` | Extend: `USE_NEW_UI` toggle that imports from `layout_v2` |
| `tests/test_recommender.py` | Extend: tests for `top_n` and `compute_disabled_options` |
| `ui/layout.py` | **DO NOT TOUCH** — rollback path |

---

## Background: Key Existing Code

**`recommender.py:161`** — `recommend_from_db(user_profile: dict) -> dict`
- Reads: `user_profile["income_usd"]`, `["preferred_countries"]`, `["lifestyle"]`, `["timeline"]`, `["nationality"]`
- Currently hard-caps at 3 cities (line 212: `if len(top_cities_raw) == 3: break`)
- Returns: `{"top_cities": [...], "overall_warning": "..."}`

**`app.py:187`** — `demo = create_layout(nomad_advisor, show_city_detail)`
- `USE_NEW_UI=1` will replace this with `demo = build_layout_v2(nomad_advisor)`

**`tests/test_recommender.py`** — 14 existing tests. Add to this file.

---

## Task 1: recommender.py — Backend Extensions

**Files:**
- Modify: `recommender.py:27-34` (add 6개월 to `_TIMELINE_FILTER`)
- Modify: `recommender.py:161` (add `top_n` param)
- Modify: `recommender.py:212` (use `top_n` instead of hardcoded 3)
- Create new function: `compute_disabled_options()` at end of file
- Modify: `tests/test_recommender.py` (add tests)

- [ ] **Step 1: Write failing tests for top_n parameter**
...
def test_6month_timeline_filters_stay_6():
    ...
    result = recommend_from_db(profile, top_n=8)
    from recommender import _visa_db, _load_data
...
```bash
pytest tests/test_recommender.py::test_top_n_8_returns_up_to_8 tests/test_recommender.py::test_6month_timeline_filters_stay_6 tests/test_recommender.py::test_compute_disabled_options_returns_dict -v
```

Expected: FAIL — `top_n` not accepted, `compute_disabled_options` not found.

- [ ] **Step 3: Add 6개월 to `_TIMELINE_FILTER` and `top_n` param**

In `recommender.py`, modify `_TIMELINE_FILTER` dict (around line 27):

```python
_TIMELINE_FILTER: dict[str, tuple[int, bool]] = {
    "90일 단기 체험":     (1,  False),
    "6개월 단기 체험":    (6,  False),   # NEW
    "1년 단기 체험":      (12, False),
    "3년 이상 장기 이민": (12, True),
    "5년 이상 장기 이민": (12, True),
}
...
Modify `recommend_from_db` signature (line 161):

```python
def recommend_from_db(user_profile: dict, top_n: int = 3) -> dict:
```

Modify the cap line (line 212):
...
- [ ] **Step 4: Add `compute_disabled_options()` to `recommender.py`**

Add this function at the end of `recommender.py` (after `recommend_from_db`):
...
def compute_disabled_options(db_profile: dict) -> dict:
    ...
    for chip_label, continent_keys in _CHIP_TO_CONTINENTS.items():
        # Build a profile where only this continent is selected
...
Expected: All tests pass including 5 new ones. Existing 14 tests still pass.

- [ ] **Step 6: Commit**

```bash
git add recommender.py tests/test_recommender.py
git commit -m "feat(recommender): add top_n param, compute_disabled_options, 6개월 timeline"
```

---

## Task 2: layout_v2.py — Python Layer (Gradio Wiring)

**Files:**
- Create: `ui/layout_v2.py`

This task builds the Python skeleton: the `filter_cities()` and `nomad_advisor_v2()` functions and the `gr.Blocks` wiring. The HTML returned by `build_ui_html()` is a placeholder stub in this task — the full HTML is built in Tasks 3–5.

- [ ] **Step 1: Create `ui/layout_v2.py` with imports and mapping constants**
...
- [ ] **Step 2: Add `filter_cities()` function**
...
- [ ] **Step 3: Add `nomad_advisor_v2()` function**

`show_city_detail(parsed_data: dict, city_index: int)` in `app.py` takes a `parsed_data` dict with
`top_cities` list and `_user_profile`, and runs the Step 2 LLM call for one city at a time.
`nomad_advisor_v2` builds that dict from the selected cities and calls it once per city.
...
def nomad_advisor_v2(payload_json: str) -> str:
    ...
    Implementation note: show_city_detail(parsed_data, city_index) in app.py expects:
      parsed_data["top_cities"]   — list of city dicts from recommend_from_db()
      parsed_data["_user_profile"] — the db_profile dict
    We call it once per selected city (city_index 0, 1, 2) and concatenate.
    """
...
- [ ] **Step 4: Add stub `build_ui_html()` and `build_layout_v2()`**
...
- [ ] **Step 5: Smoke test the Python layer manually**

```bash
python -c "
from ui.layout_v2 import filter_cities
import json
profile = json.dumps({
    'nationality': 'KR',
    ...
    'income_proof': '무증빙',
    'companion_type': '혼자',
    'spouse_income_krw': None,
    'child_age_range': None,
    'immigration_purpose': '생활비절감',
    'preparation_stage': '탐색중',
    'language_level': '영어가능'
})
result = json.loads(filter_cities(profile))
print('top_cities count:', len(result['top_cities']))
print('disabled_options keys:', list(result['disabled_options'].keys()))
print('first city:', result['top_cities'][0]['city_kr'] if result['top_cities'] else 'none')
"
```

Expected output: `top_cities count: 4` or more, `disabled_options keys: ['continents', 'timeline', 'lifestyle_tags']`

- [ ] **Step 6: Commit**

```bash
git add ui/layout_v2.py
git commit -m "feat(layout_v2): add Python layer — filter_cities, nomad_advisor_v2, gr.Blocks wiring"
```

---

## Task 3: layout_v2.py — HTML Structure & CSS

**Files:**
- Modify: `ui/layout_v2.py` — replace `build_ui_html()` stub with full HTML/CSS

Replace the stub `build_ui_html()` body with the complete HTML. This task covers structure and CSS only — JavaScript is added in Task 4.

- [ ] **Step 1: Replace `build_ui_html()` with full HTML + CSS**
...
.card-inner {
  position: relative; width: 100%; padding-top: 150%; /* 2:3 ratio */
...
<script>
// JavaScript injected in Task 4
</script>
</body>
</html>"""
```

- [ ] **Step 2: Verify HTML renders in Gradio**
...
print('Blocks created OK, components:', len(demo.blocks))
"
```

Expected: No import errors, prints component count.

- [ ] **Step 3: Commit**

```bash
git add ui/layout_v2.py
git commit -m "feat(layout_v2): add full HTML structure and iOS glass CSS"
```

---

## Task 4: layout_v2.py — JavaScript: State & Filter API

**Files:**
- Modify: `ui/layout_v2.py` — replace `// JavaScript injected in Task 4` with full JS

Replace the `<script>` tag content with the complete JavaScript. This task covers state management, filter reading, debounce, API call, and card rendering. Animations are wired up in Task 5.

- [ ] **Step 1: Add state management and filter reading JS**

Replace `// JavaScript injected in Task 4` with:
...
let state = {
  ...
  selectedCards: [],   // indices into availableCities
...
- [ ] **Step 2: Verify filter reading works (open in browser)**

Start Gradio with `USE_NEW_UI=1`:

```bash
USE_NEW_UI=1 python app.py
```

Open `http://localhost:7860`. Open browser console and run:
```js
console.log(readFilters())
```

Expected: Object with `nationality: 'KR'`, `monthly_income_krw: 3000000`, `continents: ['아시아','유럽','중남미','기타']`.

- [ ] **Step 3: Commit**

```bash
git add ui/layout_v2.py
git commit -m "feat(layout_v2): add JS state management, filter API, card rendering"
```

---

## Task 5: layout_v2.py — JavaScript: Card Animations & Card Selection

**Files:**
- Modify: `ui/layout_v2.py` — add `onCardClick()` and card reveal sequence to JS

Add the following functions to the `<script>` block, after `renderDisabled()` and before `onStep2Click()`:

- [ ] **Step 1: Add card selection and reveal sequence JS**

Insert into `<script>` block:
...
function onCardClick(idx) {
  ...
  if (alreadySelected) {
    ...
    if (state.selectedCards.length >= 3) return; // max 3
  ...
  if (newSelected.length === 3) {
    // All 3 chosen — start reveal sequence
...
Verify in browser:
1. Cards appear on load
2. Changing continent chips updates card count
3. Clicking a card gives gold border; clicking again de-selects
4. Selecting 3 cards triggers fade + flip sequence
5. "상세 비자 가이드 보기" button appears after flip

- [ ] **Step 3: Commit**

```bash
git add ui/layout_v2.py
git commit -m "feat(layout_v2): add card selection, flip reveal animation sequence"
```

---

## Task 6: app.py — USE_NEW_UI Toggle

**Files:**
- Modify: `app.py:17` (imports)
- Modify: `app.py:187` (demo creation)

- [ ] **Step 1: Add toggle to `app.py`**
...
Replace the `demo = create_layout(...)` line (line 187) with:
...
- [ ] **Step 2: Verify both modes start cleanly**

```bash
# Old UI (default)
python -c "import app; print('old UI OK:', type(app.demo))"

# New UI
USE_NEW_UI=1 python -c "import app; print('new UI OK:', type(app.demo))"
```
...
Expected: All existing tests pass. (`test_ui.py` tests `layout.py`, not `layout_v2.py` — that is fine.)

- [ ] **Step 4: Commit**

```bash
git add app.py
git commit -m "feat(app): add USE_NEW_UI toggle to switch between layout.py and layout_v2.py"
```
...
- [ ] `USE_NEW_UI=0 python app.py` → original form loads, all features work
- [ ] `USE_NEW_UI=1 python app.py` → custom UI loads at `localhost:7860`
- [ ] Filter change (chip click) → cards update within 1s
- [ ] Slider change → debounced, cards update after 300ms release
- [ ] Continent `기타` chip → UAE/Morocco/Canada cities can appear
- [ ] `6개월` chip filters correctly (different results from `1년`)
- [ ] Selecting 3 cards → flip animation plays, Step 2 button appears
- [ ] Step 2 button click → LLM result loads (may take 10–20s)
- [ ] "처음으로" button resets to filter view
- [ ] Dark mode: toggle OS dark mode, UI adapts
- [ ] `prefers-reduced-motion`: enable in OS settings, no animations play
- [ ] Mobile viewport (`< 768px`): 2-column card grid, filter panel stacked

---

## Rollback

If anything breaks, switch back instantly:

```bash
USE_NEW_UI=0 python app.py   # or remove USE_NEW_UI from .env
```

`ui/layout.py` is untouched throughout.
