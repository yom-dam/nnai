...
def test_europe_plan_triggers_wanderer():
...
def test_persona_hint_injected_in_build_prompt():
    ...
    profile = {
        ...
        "timeline": "1년 단기 체험", "persona_type": "planner",
    }
...
def test_resolve_character_defaults_to_rocky_for_missing_persona():
    assert resolve_character(None) == "rocky"
    assert resolve_character("") == "rocky"
    assert resolve_character("unknown_type") == "rocky"


def test_resolve_character_uses_persona_when_valid():
    assert resolve_character("local") == "local"
